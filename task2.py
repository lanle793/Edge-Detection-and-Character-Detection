...
def detect(img, template):
    ...
    temp_norm = normalize(template)

    i = 0
    while i < (len(img)-len(template)):
        found = False
        j = 0
        while j < (len(img[0])-len(template[0])):
            cropped_img = utils.crop(img_norm, i, i+len(template), j, j+len(template[0]))
            dif_matrix = utils.elementwise_sub(temp_norm, cropped_img)
            ssd = 0

            # calculate Sum of Square Differences
            for g in range(len(template)):
                for h in range(len(template[0])):
                    ssd += (dif_matrix[g][h] ** 2)
