...
def normalize(img):
    ...
    """
    # TODO: implement this function.
    normalized_img = np.zeros((len(img), len(img[0])))
    mx = max([max(row) for row in img])
    mn = min([min(row) for row in img])
    for i in range(len(img)):
        for j in range(len(img[0])):
            normalized_img[i][j] = (img[i][j] - mn)/(mx - mn)
    # raise NotImplementedError
...
def edge_magnitude(edge_x, edge_y):
    ...
    """
    # TODO: implement this function.
    edge_mag = np.zeros((len(edge_x), len(edge_x[0])))
